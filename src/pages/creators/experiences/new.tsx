      ...
      <form
        onSubmit={handleSubmit(async (data) => {
          ...
          const contract = await factory.deploy(
            `DerivativeWork.${data.name}`,
            "WIP",
            data.imageUrl,
            data.ticketPrice,
            data.totalSupply,
            data.lendablesUsed.map(({ lendable, monetization }: any) => {
              return {
                lendable: lendable.contractAddress,
                payment: monetization["one-time-fee"]
                  ? "OneTimeFee"
                  : "Commissions",
              };
            })
          ...
          console.log("CREATED CON", contract);
