...
contract DerivativeWork is TNT721 {
    ...
    // impl the default receive https://docs.soliditylang.org/en/v0.8.13/contracts.html#receive-ether-function
    function purchaseNextAvailable() external payable returns (bool success) {
        bool found = false;
        uint256 tokenId = 0;
        uint256 totalSupply = this.totalSupply();

        for (uint256 i = 0; i < totalSupply; i++) {
            uint256 tId = this.tokenByIndex(i);
            if (this.ownerOf(tId) != this.creator()) {
                found = true;
                tokenId = tId;
                break;
            }
        ...
        require(found, "no more tickets left to purchase");
        require(msg.value >= this.price(), "insufficient funds");
        ...
        payable(this.creator()).transfer(remainder);

        this.transferFrom(this.creator(), msg.sender, tokenId);
    ...
    constructor(
        ...
        for (uint32 i = 0; i < _totalSupply; i++) {
